...
- (instancetype)initWithCaptureSessionQueue:
...
/// All FLTCam's state access and capture session related operations should be
/// on run on this queue.
@property(strong, nonatomic) dispatch_queue_t captureSessionQueue;
/// The queue on which `latestPixelBuffer` property is accessed.
/// To avoid unnecessary contention, do not access `latestPixelBuffer` on the
/// `captureSessionQueue`.
@property(strong, nonatomic) dispatch_queue_t pixelBufferSynchronizationQueue;
/// The queue on which captured photos (not videos) are written to disk.
/// Videos are written to disk by `videoAdaptor` on an internal queue managed by
/// AVFoundation.
...
- (instancetype)initWithCameraName:(NSString*)cameraName
  ...
  @try {
    ...
    NSLog(@"MY CAMERA: Current resolution preset: %@", resolutionPreset);
  } @catch (NSError* e) {
    *error = e;
  }
  ...
  _pixelBufferSynchronizationQueue = dispatch_queue_create(
      "io.flutter.camera.pixelBufferSynchronizationQueue", NULL);
  ...
  if (_resolutionPreset == FLTResolutionPresetVeryLow ||
      _resolutionPreset == FLTResolutionPresetLow) {
    for (AVCaptureDeviceFormat* vFormat in [_captureDevice formats]) {
      NSLog(@"loop 1 yolobert");
      CMFormatDescriptionRef description = vFormat.formatDescription;
      float maxrate = ((AVFrameRateRange*)[vFormat.videoSupportedFrameRateRanges
                           objectAtIndex:0])
                          .maxFrameRate;
      ...
      if (maxrate > 29) {
        ...
        if (YES == [_captureDevice lockForConfiguration:NULL]) {
          ...
          NSLog(@"formats  %@ %@ %@", vFormat.mediaType,
                vFormat.formatDescription,
                vFormat.videoSupportedFrameRateRanges);
          break;
        }
      }
    }
...
- (void)updateOrientation:(UIDeviceOrientation)orientation
  ...
  if (connection && connection.isVideoOrientationSupported) {
    connection.videoOrientation =
        [self getVideoOrientationForDeviceOrientation:orientation];
  }
}
...
- (void)captureToFile:(FLTThreadSafeFlutterResult*)result
  ...
  AVCaptureFlashMode avFlashMode =
      FLTGetAVCaptureFlashModeForFLTFlashMode(_flashMode);
  if (avFlashMode != -1) {
  ...
  FLTSavePhotoDelegate* savePhotoDelegate = [[FLTSavePhotoDelegate alloc]
      ...
      completionHandler:^(NSString* _Nullable path, NSError* _Nullable error) {
        ...
        dispatch_async(strongSelf.captureSessionQueue, ^{
          ...
          typeof(self) strongSelf = weakSelf;
          if (!strongSelf) return;
          [strongSelf.inProgressSavePhotoDelegates
              removeObjectForKey:@(settings.uniqueID)];
        });
  ...
  NSAssert(dispatch_get_specific(FLTCaptureSessionQueueSpecific),
           @"save photo delegate references must be updated on the capture "
           @"session queue");
  self.inProgressSavePhotoDelegates[@(settings.uniqueID)] = savePhotoDelegate;
  [self.capturePhotoOutput capturePhotoWithSettings:settings
                                           delegate:savePhotoDelegate];
}
...
- (AVCaptureVideoOrientation)getVideoOrientationForDeviceOrientation:
  ...
  if (deviceOrientation == UIDeviceOrientationPortrait) {
    ...
    // Note: device orientation is flipped compared to video orientation. When
    // UIDeviceOrientation is landscape left the video orientation should be
    // landscape right.
    return AVCaptureVideoOrientationLandscapeRight;
  } else if (deviceOrientation == UIDeviceOrientationLandscapeRight) {
    // Note: device orientation is flipped compared to video orientation. When
    // UIDeviceOrientation is landscape right the video orientation should be
    // landscape left.
...
- (NSString*)getTemporaryFilePathWithExtension:(NSString*)extension
  ...
  NSString* fileDir = [[docDir stringByAppendingPathComponent:@"camera"]
      stringByAppendingPathComponent:subfolder];
  NSString* fileName =
      [prefix stringByAppendingString:[[NSUUID UUID] UUIDString]];
...
- (void)setCaptureSessionPreset:(FLTResolutionPreset)resolutionPreset {
  switch (resolutionPreset) {
    ...
    case FLTResolutionPresetUltraHigh:
      if ([_captureSession
              canSetSessionPreset:AVCaptureSessionPreset3840x2160]) {
      ...
      if ([_captureSession canSetSessionPreset:AVCaptureSessionPresetHigh]) {
        ...
        _previewSize =
            CGSizeMake(_captureDevice.activeFormat
                           .highResolutionStillImageDimensions.width,
                       _captureDevice.activeFormat
                           .highResolutionStillImageDimensions.height);
        break;
      }
...
- (void)captureOutput:(AVCaptureOutput*)output
  ...
  if (output == _captureVideoOutput) {
    ...
    // non-contest scenarios; Under rare contest scenarios, it will not block
    // for too long since the critical section is quite lightweight.
  ...
  if (_isStreamingImages) {
    ...
    if (eventSink && (self.streamingPendingFramesCount <
                      self.maxStreamingPendingFramesCount)) {
      ...
      int64_t myTime =
          CMSyncConvertTime(syncedPTS, [_captureSession masterClock],
                            originalClock)
              .value;

      // Lock the base address before accessing pixel data, and unlock it
      // afterwards. Done accessing the `pixelBuffer` at this point.
      ...
      imageBuffer[@"lensAperture"] =
          [NSNumber numberWithFloat:[_captureDevice lensAperture]];
      Float64 exposureDuration =
          CMTimeGetSeconds([_captureDevice exposureDuration]);
      Float64 nsExposureDuration = 1000000000 * exposureDuration;
      imageBuffer[@"sensorExposureTime"] =
          [NSNumber numberWithInt:nsExposureDuration];
      imageBuffer[@"timeStampNanos"] = [NSNumber numberWithLongLong:myTime];
      imageBuffer[@"sensorSensitivity"] =
          [NSNumber numberWithFloat:[_captureDevice ISO]];
    ...
    if (![_videoWriterInput appendSampleBuffer:sampleBuffer]) {
      [_methodChannel
          invokeMethod:errorMethod
             arguments:[NSString
                           stringWithFormat:@"%@",
                                            @"Unable to write to video input"]];
    }
  }
}
    ...
    if (![_audioWriterInput appendSampleBuffer:sampleBuffer]) {
      [_methodChannel
          invokeMethod:errorMethod
             arguments:[NSString
                           stringWithFormat:@"%@",
                                            @"Unable to write to audio input"]];
    }
  }
}
...
- (void)startVideoRecordingWithResult:(FLTThreadSafeFlutterResult*)result
                messengerForStreaming:
                    (nullable NSObject<FlutterBinaryMessenger>*)messenger {
  if (!_isRecording) {
    if (messenger != nil) {
    ...
    [result sendErrorWithCode:@"Error"
...
- (void)setFlashModeWithResult:(FLTThreadSafeFlutterResult*)result
...
- (void)setExposureModeWithResult:(FLTThreadSafeFlutterResult*)result
                             mode:(NSString*)modeStr {
...
- (void)applyExposureMode {
  ...
  switch (_exposureMode) {
    ...
    case FLTExposureModeAuto:
      if ([_captureDevice isExposureModeSupported:
...
- (void)setFocusModeWithResult:(FLTThreadSafeFlutterResult*)result
                          mode:(NSString*)modeStr {
  FLTFocusMode mode;
  @try {
...
- (void)applyFocusMode:(FLTFocusMode)focusMode
              onDevice:(AVCaptureDevice*)captureDevice {
...
- (void)setExposurePointWithResult:(FLTThreadSafeFlutterResult*)result
                                 x:(double)x
                                 y:(double)y {
  if (!_captureDevice.isExposurePointOfInterestSupported) {
    [result
        sendErrorWithCode:@"setExposurePointFailed"
  ...
  [_captureDevice lockForConfiguration:nil];
  [_captureDevice setExposurePointOfInterest:
                      [self getCGPointForCoordsWithOrientation:orientation
                                                             x:x
                                                             y:y]];
  [_captureDevice unlockForConfiguration];
...
- (void)setFocusPointWithResult:(FLTThreadSafeFlutterResult*)result
                              x:(double)x
                              y:(double)y {
  ...
  [_captureDevice
      setFocusPointOfInterest:[self
                                  getCGPointForCoordsWithOrientation:orientation
                                                                   x:x
                                                                   y:y]];
  [_captureDevice unlockForConfiguration];
...
- (void)setExposureOffsetWithResult:(FLTThreadSafeFlutterResult*)result
...
- (void)startImageStreamWithMessenger:
  ...
  [self startImageStreamWithMessenger:messenger
                   imageStreamHandler:
                       [[FLTImageStreamHandler alloc]
                           initWithCaptureSessionQueue:_captureSessionQueue]];
}

- (void)
    startImageStreamWithMessenger:(NSObject<FlutterBinaryMessenger>*)messenger
  ...
  if (!_isStreamingImages) {
    ...
    [threadSafeEventChannel
        setStreamHandler:_imageStreamHandler
              completion:^{
                typeof(self) strongSelf = weakSelf;
                if (!strongSelf) return;
...
- (void)stopImageStream {
  if (_isStreamingImages) {
    ...
    [_methodChannel invokeMethod:errorMethod
                       arguments:@"Images from camera are not streaming!"];
  }
}
  ...
  if (maxAvailableZoomFactor < zoom || minAvailableZoomFactor > zoom) {
    NSString* errorMessage =
        [NSString stringWithFormat:
                      @"Zoom level out of bounds (zoom level should be between "
                      @"%f and %f).",
